...
class QuartzNormalizerEngine:
    ...
    def process_folder(self, input_dir, output_dir, params, progress_cb=None):
        ...
        params: {
            ...
            'attack_rate_min': 0.0,
            'attack_rate_max': 0.0,
            'release_rate_min': 0.0,
            'release_rate_max': 0.1
        }
        """
        ...
        for i, fpath in enumerate(files):
            ...
            try:
                ...
                if target_len < 100: continue # Too short
                ...
                atk_len = int(limit_len * atk_rate * 5.0) # If user gives 0.0-0.2, usage is direct? 
                # GUI logic: "Attack Env Rate (Max 20%)" -> 0.0-1.0 slider of that 20%?
                # GUI code: max_env_len = target_len * 0.2; atk_len = int(max_env_len * atk_rate)
                # So atk_rate is 0.0-1.0 factor of the 20% chunk.
                ...
                sf.write(os.path.join(output_dir, out_name), stretched, sr)
